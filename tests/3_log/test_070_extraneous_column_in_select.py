...
@pytest.mark.parametrize("query,solutions,schema,search_path,expected_len,expected_columns", [
    ...
    (
        # existing colums in solution + non-existing columns in attempted query
        'SELECT cid, age, address FROM customer;',
        ['SELECT cid, cname FROM customer;'],
        'miedema',
        'miedema',
        (3, 2),
        [('miedema', None, 'age'), ('miedema', None, 'address')],
    ),
    (
...
@pytest.mark.parametrize("query,solutions,schema", [
    ...
    (
        # too few columns in attempted query
